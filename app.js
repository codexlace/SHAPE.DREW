...
const littleObjectsByLane = {
  object: ['tiny key', 'soft heart', 'paper tag', 'warning label', 'tiny spoon', 'button', 'ribbon bow', 'glow dot', 'little envelope', 'mini sign', 'small bell', 'single sparkle', 'moon charm', 'crumb', 'sticker patch'],
  food: ['soft heart', 'crumb', 'tiny fork', 'tiny spoon', 'ribbon bow', 'little cherry', 'single sparkle', 'glow dot', 'mini crown', 'butter pat', 'little napkin tag', 'mini sign', 'question mark patch', 'small bell', 'sticker patch'],
  symbol: ['arrow label', 'tiny crown', 'single sparkle', 'moon charm', 'tiny flag', 'warning label', 'button badge', 'little eye charm', 'mini sign', 'glow dot', 'ribbon bow', 'small bell', 'folded note', 'soft heart', 'tiny key'],
  ghost: ['small lantern', 'soft heart', 'tiny moon', 'glow dot', 'single sparkle', 'tiny bell', 'little mask', 'ribbon bow', 'mini sign', 'cloud puff', 'crumb', 'small key', 'moon charm', 'sticker patch', 'tiny crown'],
  stationery: ['tiny pencil', 'tiny brush', 'paper clip', 'button', 'warning label', 'ribbon bow', 'sticky note', 'folded note', 'sealed envelope', 'mini sign', 'small bell', 'glow dot', 'single sparkle', 'soft heart', 'question mark patch'],
  weather: ['tiny umbrella', 'raindrop', 'cloud puff', 'single sparkle', 'moon charm', 'glow dot', 'tiny rainbow shard', 'small bell', 'soft heart', 'mini sign', 'ribbon bow', 'question mark patch', 'crumb', 'button', 'tiny crown'],
  plant: ['little flower', 'seed packet', 'watering drop', 'soft heart', 'ribbon bow', 'small bell', 'glow dot', 'tiny mushroom', 'mini sign', 'single sparkle', 'moon charm', 'button', 'crumb', 'little envelope', 'tiny crown'],
  charm: ['tiny crown', 'small bell', 'moon charm', 'soft heart', 'tiny key', 'button badge', 'ribbon bow', 'glow dot', 'single sparkle', 'mini sign', 'sealed envelope', 'little flower', 'sticker patch', 'question mark patch', 'crumb']
};

const allLittleObjects = [...new Set([...Object.values(littleObjectsByLane).flat(), ...extras])];

const littleObjectHints = {
  surprise: 'Choose a little object yourself, or let the drawer toss one in.',
  object: 'Object gremlins like one readable little object attached or floating beside them.',
  food: 'Snack creatures work best with one simple object, not a whole meal scene.',
  symbol: 'Symbol creatures like tiny badges, arrows, crowns, and sign-like extras.',
  ghost: 'Soft monsters like floaty, glowy, slightly haunted little objects.',
  stationery: 'Stationery oddlets love desk-tool extras, labels, and clipped-on nonsense.',
  weather: 'Weather blobs work best with tiny floating weather bits or one symbolic object.',
  plant: 'Plant oddlets like small nature props, seeds, tags, bells, and light magical extras.',
  charm: 'Charm oddlets want icon-like little objects that feel collectible and clipped-on.'
};
...
function updateObjectSelect() {
  ...
  if (laneValue === 'surprise') {
    options = allLittleObjects;
  } else {
    options = littleObjectsByLane[laneValue] || allLittleObjects;
  }
...
function littleObjectToBlueprintZone(littleObject, speciesRules = null) {
  ...
  if (text.includes('crown') || text.includes('hat') || text.includes('halo')) return 'topCenter';
  if (text.includes('bell') || text.includes('charm')) return 'topLoop';
  if (text.includes('ribbon') || text.includes('bow') || text.includes('tag')) return 'sideAttach';
  if (text.includes('label') || text.includes('badge') || text.includes('sticker') || text.includes('patch')) return 'labelPatch';
  if (text.includes('sign') || text.includes('certificate') || text.includes('note')) return 'frontCenter';
  if (text.includes('spark') || text.includes('star') || text.includes('moon') || text.includes('glow')) return 'upperRight';
  if (text.includes('puddle') || text.includes('shadow')) return 'underBody';
  if (text.includes('button')) return speciesRules?.bodyShape === 'circleButton' ? 'buttonHole' : 'surfacePatch';
  if (text.includes('flower') || text.includes('key') || text.includes('spoon') || text.includes('envelope') || text.includes('crumb') || text.includes('drop') || text.includes('cloud') || text.includes('umbrella')) return 'sideFloat';
  return speciesRules?.propAnchor || 'sideFloat';
}

function littleObjectMarkerLabel(littleObject) {
  const text = String(littleObject || 'object').toLowerCase().trim();
  const cleaned = text
    .replace(/\b(tiny|small|little|mini|soft|single)\b/g, '')
    .replace(/\s+/g, ' ')
    .trim();
  const words = cleaned.split(' ').filter(Boolean);
  return escapeHtml((words.slice(-2).join(' ') || 'object').slice(0, 16));
}

function littleObjectPlacementNote(littleObject, zone) {
  const name = titleCase(String(littleObject || 'little object'));
  const zoneNotes = {
    topCenter: 'Place it on top so it reads before the face starts competing.',
    topLoop: 'Attach it to the top loop or hanger area so it feels clipped on.',
    sideAttach: 'Attach it to one side edge so it reads as part of the body.',
    labelPatch: 'Put it on the body surface like a readable label or badge.',
    frontCenter: 'Place it at the front so it reads like a held or presented object.',
    upperRight: 'Float it slightly above or beside the mascot as a light accent object.',
    underBody: 'Place it below the mascot as a grounding object or base detail.',
    buttonHole: 'Anchor it near the button-hole area so the placement feels intentional.',
    sideFloat: 'Keep it beside the body so it feels like a companion object, not a whole scene.',
    surfacePatch: 'Place it on the clearest body patch so it stays readable.'
  };
...
function rollCard({ fullSurprise = false, mutate = null, daily = false } = {}) {
  ...
  const objectChoice = $('#objectSelect')?.value || 'surprise';
  const laneObjectPool = littleObjectsByLane[lane] || allLittleObjects;
  const mergedObjectPool = [...new Set([...laneObjectPool, ...extraPool])];
  const extra = objectChoice !== 'surprise' ? objectChoice : choice(mergedObjectPool);
...
function chooseDailyControls() {
  ...
  const dailyObjects = littleObjectsByLane[$('#laneSelect').value] || allLittleObjects;
  $('#objectSelect').value = dailyObjects.length ? seededPick(dailyObjects.map((item) => ({ value: item })), `${today}:object`).value : 'surprise';
...
function littleObjectPreviewKind(littleObject) {
  const text = String(littleObject || '').toLowerCase();
  if (text.includes('crown')) return 'crown';
  if (text.includes('ribbon') || text.includes('bow')) return 'bow';
  if (text.includes('label') || text.includes('badge') || text.includes('sticker') || text.includes('patch') || text.includes('tag')) return 'badge';
  if (text.includes('sign') || text.includes('certificate') || text.includes('note')) return 'sign';
  if (text.includes('spark') || text.includes('star')) return 'spark';
  if (text.includes('moon')) return 'moon';
  if (text.includes('heart')) return 'heart';
  if (text.includes('spoon')) return 'spoon';
  ...
  if (text.includes('drop') || text.includes('raindrop') || text.includes('paint drop') || text.includes('watering drop')) return 'drop';
  if (text.includes('umbrella')) return 'umbrella';
  if (text.includes('cloud')) return 'cloud';
  if (text.includes('brush') || text.includes('pencil')) return 'brush';
  if (text.includes('crumb')) return 'crumb';
  return 'dot';
}

function renderLittleObjectPreviewSvg(littleObject, zone, colors) {
  const point = blueprintPoint(zone);
  const kind = littleObjectPreviewKind(littleObject);
  const p = {
    crown: `<path d="M-12 7 L-9 -5 L-1 0 L0 -9 L7 0 L14 -5 L12 7 Z" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2" stroke-linejoin="round"/><circle cx="-9" cy="-5" r="1.5" fill="${colors.mint}"/><circle cx="0" cy="-9" r="1.5" fill="${colors.mint}"/><circle cx="14" cy="-5" r="1.5" fill="${colors.mint}"/>`,
    bow: `<path d="M-14 0 C-20 -8 -10 -14 -2 -7 L0 -2 L-2 7 C-10 14 -20 8 -14 0Z" fill="rgba(255,255,255,.08)" stroke="${colors.pink}" stroke-width="2"/><path d="M14 0 C20 -8 10 -14 2 -7 L0 -2 L2 7 C10 14 20 8 14 0Z" fill="rgba(255,255,255,.08)" stroke="${colors.pink}" stroke-width="2"/><circle cx="0" cy="0" r="4" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/>`,
    badge: `<rect x="-12" y="-9" width="24" height="18" rx="5" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/><path d="M-6 0 H6" stroke="${colors.mint}" stroke-width="2" stroke-linecap="round"/>`,
    sign: `<rect x="-12" y="-10" width="24" height="16" rx="4" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/><path d="M0 6 V14" stroke="${colors.primary}" stroke-width="2" stroke-linecap="round"/>`,
    spark: `<path d="M0 -12 L4 -4 L12 0 L4 4 L0 12 L-4 4 L-12 0 L-4 -4 Z" fill="rgba(255,255,255,.08)" stroke="${colors.mint}" stroke-width="2" stroke-linejoin="round"/>`,
    moon: `<path d="M3 -12 C-7 -11 -12 -2 -9 7 C-6 15 3 18 11 12 C5 11 1 6 1 0 C1 -5 3 -9 7 -12 C6 -12 4 -12 3 -12Z" fill="rgba(255,255,255,.08)" stroke="${colors.blue}" stroke-width="2"/>`,
    heart: `<path d="M0 12 C-14 2 -16 -8 -8 -11 C-4 -13 -1 -10 0 -7 C1 -10 4 -13 8 -11 C16 -8 14 2 0 12Z" fill="rgba(255,255,255,.08)" stroke="${colors.pink}" stroke-width="2"/>`,
    spoon: `<ellipse cx="-4" cy="-3" rx="7" ry="9" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/><path d="M1 3 L12 14" stroke="${colors.primary}" stroke-width="3" stroke-linecap="round"/>`,
    key: `<circle cx="-5" cy="0" r="6" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/><path d="M1 0 H13 M8 0 V5 M11 0 V4" stroke="${colors.primary}" stroke-width="2" stroke-linecap="round"/>`,
    bell: `<path d="M-10 6 C-6 2 -7 -6 -2 -10 C1 -12 5 -12 8 -10 C13 -6 12 2 16 6 Z" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2" stroke-linejoin="round"/><path d="M-7 9 H13" stroke="${colors.mint}" stroke-width="2" stroke-linecap="round"/>`,
    envelope: `<rect x="-12" y="-8" width="24" height="16" rx="4" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/><path d="M-10 -6 L0 1 L10 -6 M-10 6 L-1 -1 M10 6 L1 -1" stroke="${colors.mint}" stroke-width="1.75" fill="none" stroke-linecap="round"/>`,
    button: `<circle cx="0" cy="0" r="10" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/><circle cx="-3" cy="-3" r="1.5" fill="${colors.mint}"/><circle cx="3" cy="-3" r="1.5" fill="${colors.mint}"/><circle cx="-3" cy="3" r="1.5" fill="${colors.mint}"/><circle cx="3" cy="3" r="1.5" fill="${colors.mint}"/>`,
    flower: `<circle cx="0" cy="0" r="4" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/><circle cx="0" cy="-9" r="4" fill="rgba(255,255,255,.08)" stroke="${colors.pink}" stroke-width="2"/><circle cx="8" cy="-2" r="4" fill="rgba(255,255,255,.08)" stroke="${colors.pink}" stroke-width="2"/><circle cx="5" cy="8" r="4" fill="rgba(255,255,255,.08)" stroke="${colors.pink}" stroke-width="2"/><circle cx="-5" cy="8" r="4" fill="rgba(255,255,255,.08)" stroke="${colors.pink}" stroke-width="2"/><circle cx="-8" cy="-2" r="4" fill="rgba(255,255,255,.08)" stroke="${colors.pink}" stroke-width="2"/>`,
    drop: `<path d="M0 -12 C7 -2 12 4 11 9 C10 15 5 18 0 18 C-5 18 -10 15 -11 9 C-12 4 -7 -2 0 -12Z" fill="rgba(255,255,255,.08)" stroke="${colors.blue}" stroke-width="2"/>`,
    umbrella: `<path d="M-12 0 Q0 -12 12 0 Z" fill="rgba(255,255,255,.08)" stroke="${colors.blue}" stroke-width="2"/><path d="M0 0 V12 Q0 16 5 14" stroke="${colors.primary}" stroke-width="2" stroke-linecap="round" fill="none"/>`,
    cloud: `<path d="M-12 4 C-14 -2 -8 -6 -3 -5 C-1 -10 8 -10 10 -4 C15 -4 18 1 15 5 C12 9 -8 10 -12 4Z" fill="rgba(255,255,255,.08)" stroke="${colors.blue}" stroke-width="2"/>`,
    brush: `<path d="M-10 7 L2 -5" stroke="${colors.primary}" stroke-width="3" stroke-linecap="round"/><path d="M2 -5 L11 -12" stroke="${colors.mint}" stroke-width="3" stroke-linecap="round"/><path d="M-12 9 L-6 15" stroke="${colors.primary}" stroke-width="3" stroke-linecap="round"/>`,
    crumb: `<circle cx="-6" cy="2" r="3" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/><circle cx="2" cy="-3" r="2.5" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/><circle cx="8" cy="4" r="2" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/>`,
    dot: `<circle cx="0" cy="0" r="7" fill="rgba(255,255,255,.08)" stroke="${colors.primary}" stroke-width="2"/>`
  };
  return `<g transform="translate(${point.x} ${point.y}) scale(0.9)" opacity="0.95">${p[kind] || p.dot}</g>`;
}
...
Little object: ${card.extra || 'surprise'}
Little object placement: ${littleObjectPlacementNote(card.extra, littleObjectToBlueprintZone(card.extra, card.blueprintSpecies || inferSpeciesBlueprint(card.species || card.mascot, card.lane)))}
...
- Little object placement: ${blueprint.propAnchor}
